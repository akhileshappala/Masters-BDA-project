...
function useDataTableInstance(columns, data, options, hooks) {
  ...
  const normalizedColumns = useMemo(() => columns.map(col => {
    const {
      sortBy,
      sortType,
      ...columnProps
  ...
  const filterColumns = useMemo(() => allColumns.filter(it => it.canFilter && it.Filter).map(props => props), [allColumns]);
  const resetFilter = useCallback(columnId => setFilter(columnId, undefined), [setFilter]);
  ...
  const getAllHeadersInAGroup = useCallback((curr, headerGroupid) => allColumns.filter(col => col.id === 'selector' || !headerGroupid || (col.parent ? col.parent.id === headerGroupid : false)).map(col => {
