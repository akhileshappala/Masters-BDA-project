...
export default function useTreeViewDescendants({
  ...
  const registeredChildren = useRef([]);
  ...
  const descendants = useMemo(() => registeredChildren.current, [registeredChildren.current]);
  /* eslint-disable react-hooks/exhaustive-deps */
  ...
  const registerDescendant = useCallback((id, children, descendantElement) => {
    let newIndex = -1;
  ...
  const updateDescendant = useCallback((id, children, descendantElement) => {
    if (registeredChildren.current) {
      const currentIndex = _findIndex(registeredChildren.current, node => node.id === id);
  ...
  const updateCheckboxSelection = useCallback((event, descendantId, checkedArray, uncheckedArray) => {
    if (registeredChildren.current) {
