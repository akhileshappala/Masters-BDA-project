...
state_table_extraction = PythonOperator(task_id='state_table_extraction',python_callable=run_glue_job,op_kwargs={'job_name':'state_tracker_data_extractor'},dag=dag)
state_dim_load_ready = PythonOperator(task_id='state_dim_load_ready',python_callable=run_glue_job,op_kwargs={'job_name':'state_dim_data_load_ready'},dag=dag)
...
date_dim_load_ready = PythonOperator(task_id='date_dim_load_ready',python_callable=run_glue_job,op_kwargs={'job_name':'date_dim_load_ready_2'},dag=dag)
date_dim_load = PythonOperator(task_id='date_dim_load',python_callable=run_glue_job,op_kwargs={'job_name':'date_dim_load'},dag=dag)

covid_raw_extract_full = PythonOperator(task_id='covid_raw_extract_full',python_callable=run_glue_job,op_kwargs={'job_name':'covid_raw_data_state_partitioned'},dag=dag)
covid_fact_load_ready = PythonOperator(task_id='covid_fact_load_ready',python_callable=run_glue_job,op_kwargs={'job_name':'covid_fact_data_load_ready'},dag=dag)
